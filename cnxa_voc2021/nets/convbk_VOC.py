...
class BIGKS(nn.Module):
    def __init__(self, channel, ratio=8, kernel_size=[21, 19], stride=1, I=0, N=0, pad='', use_gpu=True):
        ...
        self.bk1 = nn.Sequential(
            nn.Conv2d(channel, channel // N, kernel_size=k1, padding=p1, stride=stride),
            nn.ReLU(),
            nn.Conv2d(channel // N, channel // N, kernel_size=3, padding=1, stride=1)
        )
        ...
        # self.des1 = nn.Conv2d(self.channel, self.channel, kernel_size=6, padding=0, stride=1)
        # self.des2 = nn.Conv2d(self.channel, self.channel, kernel_size=7, padding=0, stride=1)
    ...
    def forward(self, x):
        ...
        # x2 = self.des2(x_bk1_c)
        avg_bk2 = self.max_pool(x_bk2).view([b, c])
        avg_bk1 = self.max_pool(x_bk1_c).view([b, c])

        x1 = self.fc(avg_bk1).view([b, c, 1, 1])

        x2 = self.fc(avg_bk2).view([b, c, 1, 1])
        # y = self.sigmoid(x1) + self.sigmoid(x2)
...
class ConvBK(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 layer_scale_init_value=1e-6, head_init_scale=1., pad='', use_gpu=True
                 ):
        super().__init__()

        self.bk = BIGKS(channel=dims[-1], I=13, N=4, pad=pad, use_gpu=use_gpu)
...
def convbk(num_classes: int, pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        weights_dict = \
            torch.load("D://PyCharm/xiaozhijie/modify_project/CNX_ImageNet100/model_data/convnext_tiny_1k_224_ema.pth")["model"]
        # 删除有关分类类别的权重
...
def convbk_base(num_classes: int, pretrained=False, pad='', use_gpu=True, **kwargs):
    model = ConvBK(depths=[3, 3, 27, 3],
                   dims=[128, 256, 512, 1024],
                   num_classes=num_classes,
                   pad=pad, use_gpu=use_gpu)
    if pretrained:
        # model.load_state_dict(torch.load("/data/CNX_V3/model_data/convnext_base_1k_224_ema.pth")["model"])
