...
if __name__ == "__main__":
    ...
    classes_path = 'model_data/voc2021_classes.txt'
    # ----------------------------------------------------#
    #   输入的图片大小
    # ----------------------------------------------------#
    input_shape = [300, 300]
    # ----------------------------------------------------#
    #   所用模型种类：
    #   resnet50、vgg16、vit、convnext、convnextaa
    #
    ...
    backbone = "convbk_dw"
    # ----------------------------------------------------#
    #   在convnext中嵌入的注意力机制(convnextaa)：
    #   ca、sa、senet、ecanet
    #
    #   想使用哪个注意力机制则将aa设置成谁,若使用ca 则 aa = "ca"
    #   若使用的是vit 或者 vgg、resnet、convnext等网络则置空，即 aa = ""
    # ----------------------------------------------------#
    aa = ""
    pad = 'avg'
    print("====> Prepare start train {}{}!".format(backbone, aa))
    # ----------------------------------------------------------------------------------------------------------------------------#
    ...
    #   获得图片路径和标签, 使用对应数据集的
    # ------------------------------------------------------#
    annotation_path = "cls_train_voc2021.txt"
    annotation_path_ = "cls_test_voc2021.txt"
    # ------------------------------------------------------#
    ...
    if backbone != "vit":
        model = get_model_from_name[backbone](num_classes=num_classes, pretrained=pretrained, aa=aa, use_gpu=Cuda, pad=pad)
    # elif backbone == "convnextaa_base":
        # model = get_model_from_name[backbone](num_classes=num_classes, pretrained=pretrained, aa=aa)
    else:
    ...
    if model_path != "":
        ...
        if backbone == "vit" or backbone == "resnet50" or backbone == "vgg16":
    ...
    with open(annotation_path, "r") as f:
        lines = f.readlines()
    with open(annotation_path_, "r") as f:
        lines_ = f.readlines()
    ...
    if True:
        ...
        test_dataset = DataGenerator(lines_, input_shape, False)
        ...
        gen_val = DataLoader(val_dataset, batch_size=Batch_size, num_workers=num_workers, pin_memory=True,
                             drop_last=True, collate_fn=detection_collate)
        gen_test = DataLoader(test_dataset, batch_size=Batch_size, num_workers=num_workers, pin_memory=True,
                             drop_last=True, collate_fn=detection_collate)
        # ------------------------------------#
        #   冻结一定部分训练
        # ------------------------------------#
        ...
        for epoch in range(Init_Epoch, Freeze_Epoch):
            fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val,
                          Freeze_Epoch, Cuda, backbone, aa, gen_test)
            lr_scheduler.step()
    ...
    if True:
        ...
        for epoch in range(Freeze_Epoch, Epoch):
            fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val,
                          Epoch, Cuda, backbone, aa, gen_test)
            lr_scheduler.step()
